...
public class PicModeManagerImpl extends PicModeManagerAbs {
    ...
    private int getBacklight() {
        int ret = -1;
        String productType = SystemProperties.get(SettingManager.SYSPROP_PRODUCTMODEL);
        if (productType.contains(TYPE_PROJ)) {
            Log.i(TAG, "GetProjectorBacklightMode ");
            ret = mPicSetManager.GetProjectorBacklightMode();
        } else {
            ret = mPicSetManager.getBacklight();
        }
        Log.i(TAG, "the backlight for current source is " + ret);
        return ret;
    }

    private boolean setBacklight(int val) {
        boolean ret = false;
        String productType = SystemProperties.get(SettingManager.SYSPROP_PRODUCTMODEL);
        if (productType.contains(TYPE_PROJ)) {
            ret = mPicSetManager.SetProjectorBacklightModeSafety(val);
            if (ret) {
                ret = mPicSetManager.SetProjectorBacklightMode(val);
            }
            Log.i(TAG, "SetProjectorBacklightMode " + val);
        } else {
            ret = mPicSetManager.setBacklight(val);
        }
